...
def get_stats(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Recent ontologies
    ontology_query = db.query(OntologyProject)
    if current_user.role != "admin":
        ontology_query = ontology_query.filter(OntologyProject.created_by == current_user.id)
    visible_ids = [row.id for row in ontology_query.all()]
    recent = ontology_query.order_by(OntologyProject.updated_at.desc()).limit(6).all()
    recent_list = []
    for o in recent:
    ...
    # Domain distribution
    domain_rows = (db.query(OntologyProject.domain, func.count(OntologyProject.id))
                   .filter(OntologyProject.id.in_(visible_ids))
                   .group_by(OntologyProject.domain).all())
    domain_counts = {row[0]: row[1] for row in domain_rows if row[0]}

    # Status breakdown
    status_rows = (db.query(OntologyProject.status, func.count(OntologyProject.id))
                   .filter(OntologyProject.id.in_(visible_ids))
                   .group_by(OntologyProject.status).all())
    status_counts = {row[0]: row[1] for row in status_rows if row[0]}

    return {
        "data": {
            "ontology_count": len(visible_ids),
            "entity_count": db.query(Entity).filter(Entity.ontology_id.in_(visible_ids)).count(),
            "logic_count": sum(_combined_count(db, LogicRule, OntologyLogicRule, oid) for oid in visible_ids),
            "action_count": sum(_combined_count(db, Action, OntologyActionType, oid) for oid in visible_ids),
